...
def create_icmp_packet(packet_id):
    ...
    # ICMP类型为8（回显请求），我们将标识符设为packet_id，序列号设为1r
...
# 接收ICMP回显应答并处理
def receive_one_ping(sock, packet_id, timeout, dest_addr):
    """
...
def do_one_ping(dest_addr, timeout=1):
    ...
    send_one_ping(sock, dest_addr, packet_id)  # 发送ping请求
    delay = receive_one_ping(sock, packet_id, timeout, dest_addr)  # 接收ping回复
...
def ping(dest_addr, timeout=1, count=4):
    ...
    try:
        ...
        print(f"正在 Ping {dest_addr} [{dest_ip}]:")
        ...
        if rtt_list:
            ...
            print(
                f"往返时间最小/平均/最大 = {min(rtt_list):.3f}/{sum(rtt_list) / len(rtt_list):.3f}/{max(rtt_list):.3f} ms")
        else:
            print(f"\n--- {dest_addr} ping 统计 ---")
            print(f"没有收到任何回复。")
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--host', required=True, help="目标主机名或IP地址")
    ...
    # 传递用户输入的参数给 ping 函数
    # ping(args.host, args.timeout, args.count)
